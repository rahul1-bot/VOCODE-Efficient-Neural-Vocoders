# This module:
# 1. Implements the APNet2 vocoder for the Study 1 reproduction cohort,
#    following the redmist328 recipe: parallel amplitude and phase
#    spectrum predictors whose outputs combine through an inverse STFT
#    into the waveform in one frame-level pass
# 2. Trains adversarially with the APNet2 loss composition: amplitude,
#    anti-wrapping phase, reconstruction, and consistency terms plus the
#    discriminator objectives
# 3. Defines Apnet2Config, the frozen experiment record pinning the
#    topology, the optimization settings, and the two mel protocols the
...
# - Validation computes the spectral validation loss on the predicted
#   components and logs val_loss for checkpoint monitoring; predict_step and
...
#
# Design decisions:
# - Training and validation operate on the component predictions
#   (log-amplitude and phase) rather than only the waveform, because the
#   reference objective supervises the spectral streams directly
# - The center-true conditioning protocol of the reference is preserved
#   in float32 under a disabled-autocast context
# - Gradient clipping is applied directly to each parameter group inside its
#   own update rather than delegated to the harness clipping surface, so the
#   generator and the discriminator ensembles are clipped independently
#
...
__all__: list[str] = ["Apnet2", "Apnet2Config"]


class Apnet2Config(BaseModel):
    # Frozen experiment record of one APNet2 run. Every value the module and
    ...
    #
    # This record is the architecture recipe of the APNet2 Project-Trained
    ...
    #         input width of both stream entry projections.
    #     n_fft: Transform size of the synthesis STFT; the one-sided bin
    #         count is the output width of every spectral head.
    ...
    #     convnext_layer_count: Residual block depth of each stream.
    #         Default: ``8``.
    ...
    #         Default: ``1000.0``.
    #     loss_configuration: Term weights of the APNet2 loss composition.
    #         Default: ``Apnet2LossConfig()``.
    #     mel_protocol: Conditioning mel protocol; the reference conditions
    #         on a band-limited mel.
    ...
    asp_channel: PositiveInt
    psp_channel: PositiveInt
    asp_input_conv_kernel_size: PositiveInt
    asp_output_conv_kernel_size: PositiveInt
    ...
    loss_configuration: Apnet2LossConfig = Apnet2LossConfig()
    ...
    @classmethod
    def redmist328(cls) -> Apnet2Config:
        # Builds the APNet2 configuration aligned with the redmist328 reference implementation.
        #
        ...
        # declared defaults, which already match the reference.
        return cls(
            ...
            mel_protocol=MelConfig.apnet2_redmist328(),
            reconstruction_mel_protocol=MelConfig.apnet2_redmist328_reconstruction(),
            pesq_protocol=PesqConfig(),
            stoi_protocol=StoiConfig()
        )
...
class Apnet2(Module):
    # The APNet2 vocoder as a harness module. It owns the generator network,
    # the two discriminator ensembles, the loss composition, and the two mel
    # transforms, and it drives adversarial training itself because automatic
    ...
    # parameters, so neither update can leave gradients in the other's
    # weights.
    #
    # Every spectral quantity that enters a loss is extracted inside a
    # float32 island: the conditioning mel, the full-band reconstruction mel,
    # and the STFT targets are all computed with autocast disabled, because the
    # reference protocol is what the published checkpoint was trained under and
    # reduced precision would silently change it.
    #
    # Integration: the module is constructed through the model registry from
    # an Apnet2Config, and the published redmist328 release is loaded onto its
    # network attribute by vocode.models.apnet2.weights.Apnet2Weights. It
    ...
    def __init__(self, configuration: Apnet2Config) -> None:
        ...
        self._configuration: Apnet2Config = configuration
        ...
        self.network: Apnet2Network = Apnet2Network(
            num_mels=configuration.input_mel_channels,
            n_fft=configuration.n_fft,
            hop_size=configuration.hop_size,
            win_size=configuration.win_size,
            asp_channel=configuration.asp_channel,
            psp_channel=configuration.psp_channel,
            asp_input_conv_kernel_size=configuration.asp_input_conv_kernel_size,
            asp_output_conv_kernel_size=configuration.asp_output_conv_kernel_size,
        ...
        self._loss: Apnet2Loss = Apnet2Loss(configuration.loss_configuration)
    ...
    def training_step(self, batch: Batch, batch_idx: int) -> StepOutput:
        ...
        # the generator update through the full amplitude, phase,
        # reconstruction, consistency, and adversarial composition;
        # components are logged and the detached generator loss returned.
        #
        ...
        generator_output: Apnet2GeneratorOutput = self.network.predict_components(reference_mel)
    ...
    @property
    def configuration(self) -> Apnet2Config:
    ...
    def _run_generator_training_step(
        ...
        generator_output: Apnet2GeneratorOutput,
        real_waveform: torch.Tensor,
        fake_waveform: torch.Tensor
    ) -> tuple[torch.Tensor, dict[str, float]]:
    ...
    def _require_gan_optimizers(self) -> tuple[torch.optim.Optimizer, torch.optim.Optimizer]:
        ...
        if not isinstance(optimizers, list) or len(optimizers) != 2:
            raise RuntimeError("APNet2 training requires generator and discriminator optimizers")
