# This module:
# 1. Loads the author-released redmist328 APNet2 checkpoint onto the project
#    APNet2 implementation as the published reference anchor:
...
#   names, because the project network reproduces those names exactly; a
#   remapping layer here would be able to hide a genuine divergence from
...
from vocode.models.apnet2.apnet2 import Apnet2
...
__all__: list[str] = ["Apnet2Weights"]


class Apnet2Weights(PublishedWeights):
    # Published-weights adapter for APNet2. It binds one immutable release
    ...
    # registered checkpoint gate targets. It is distinct from the APNet2
    ...
    def __init__(self) -> None:
        ...
        self._provenance: PublishedWeightProvenance = PublishedWeightProvenance(
            ...
            variant_name="redmist328_ljspeech_g_01000000",
            author_source_uri="http://home.ustc.edu.cn/~redmist/APNet2/",
            retrieval_kind="huggingface_file",
            retrieval_uri="r-sawhney/vocode-phase1-assets",
            retrieval_filename="apnet2_lj/g_01000000",
            expected_sha256="85e447121cb2e4eb5ed52e7bd7be285633ce6d789a471a1301950c32e37898cf",
            local_relative_path=Path("apnet2_lj/g_01000000"),
    ...
    def load(self, module: Module, published_weights_root: Path) -> None:
        ...
        #
        # Args:
        #     module: The harness module to load onto. It must be an Apnet2,
        ...
        #
        # Raises:
        #     TypeError: If the module is not an Apnet2, or the deserialized
        ...
        #         project network disagree on any parameter name or shape.
        #         This is the substantive reproduction failure the adapter
        #         exists to surface, and it is deliberately not softened to a
        #         relaxed load.
        if not isinstance(module, Apnet2):
            raise TypeError(
                f"APNet2 published weights require Apnet2, got {type(module).__name__}. "
        ...
        network: Apnet2Network = module.network
        network.load_state_dict(state_dict, strict=True)
        log.info(f"Loaded strict author weights for apnet2 from {local_path}")
    ...
    def _ensure_local_path(self, published_weights_root: Path) -> Path:
        ...
        #
        # The recorded retrieval filename already carries the release's own
        # directory prefix, so the download is directed at the weights root and
        # reproduces the recorded relative layout in place; the copy afterwards
        # only runs when the client resolved the file somewhere else, for
        # instance through a shared cache. Verification runs unconditionally at
        # the end, so an existing cache entry is re-checked on every load and
        # never trusted merely for existing.
        #
        # Args:
        #     published_weights_root: Local root the release is cached under.
        #
        ...
        if not local_path.exists():
            ...
            if source_filename is None:
                raise ValueError("APNet2 retrieval filename is missing.")
    ...
    def _validate_sha256(self, local_path: Path) -> None:
        ...
        if not local_path.is_file():
            raise FileNotFoundError(f"APNet2 published weight file is missing at {local_path}.")
        handle: BinaryIO
        with local_path.open("rb") as handle:
            actual_sha256: str = hashlib.file_digest(handle, "sha256").hexdigest()
        if actual_sha256 != self._provenance.expected_sha256:
            raise ValueError(
                f"APNet2 weight hash mismatch at {local_path}: "
    ...
    def _extract_state_dict(self, loaded_checkpoint: object) -> dict[str, torch.Tensor]:
        ...
        if not isinstance(loaded_checkpoint, Mapping):
            raise TypeError(
                f"APNet2 author checkpoint must be a mapping, got "
        ...
        if not isinstance(raw_state_dict, Mapping):
            raise TypeError(
                f"APNet2 author checkpoint must contain a mapping at generator, got "
        ...
        for raw_key, raw_value in raw_state_dict.items():
            ...
            if not isinstance(raw_value, torch.Tensor):
                raise TypeError(
                    f"APNet2 state entry {key} must be torch.Tensor, got "
