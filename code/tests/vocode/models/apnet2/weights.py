# This module:
# 1. Verifies the APNet2 published-weight provenance record: the author
...
class Apnet2PublishedWeightProvenanceTest(unittest.TestCase):
    # Verifies the recorded release identity of the redmist328 APNet2 checkpoint.
    def setUp(self) -> None:
        # Builds the weight adapter and reads its provenance record.
        self._adapter: Apnet2Weights = Apnet2Weights()
        self._provenance: PublishedWeightProvenance = self._adapter.provenance

    def test_release_identity_matches_the_redmist_publication(self) -> None:
        # The reference anchor is the author's one-million-step LJSpeech generator.
        self.assertEqual(self._provenance.architecture_name, "apnet2")
        self.assertEqual(self._provenance.variant_name, "redmist328_ljspeech_g_01000000")
        self.assertEqual(self._provenance.author_source_uri, "http://home.ustc.edu.cn/~redmist/APNet2/")
        self.assertEqual(self._provenance.retrieval_kind, "huggingface_file")
        self.assertEqual(self._provenance.retrieval_filename, "apnet2_lj/g_01000000")
    ...
    def test_recorded_digest_and_serialization_are_pinned(self) -> None:
        ...
        self.assertEqual(
            ...
            "85e447121cb2e4eb5ed52e7bd7be285633ce6d789a471a1301950c32e37898cf"
        )
    ...
    def test_local_path_resolves_under_the_weights_root(self) -> None:
        ...
        self.assertEqual(self._provenance.resolve_local_path(root), root / Path("apnet2_lj/g_01000000"))
...
class Apnet2CheckpointValidationTest(unittest.TestCase):
    ...
    def test_non_tensor_entry_is_rejected(self) -> None:
        ...
        with self.assertRaises(TypeError):
            self._adapter._extract_state_dict({"generator": {"ASP_input_conv.weight": 5}})
    ...
    def test_generator_entries_are_extracted_under_their_keys(self) -> None:
        ...
        payload: dict[str, dict[str, torch.Tensor]] = {"generator": {"ASP_input_conv.bias": torch.zeros(3)}}
        extracted: dict[str, torch.Tensor] = self._adapter._extract_state_dict(payload)
        self.assertEqual(list(extracted.keys()), ["ASP_input_conv.bias"])
        self.assertTrue(torch.equal(extracted["ASP_input_conv.bias"], payload["generator"]["ASP_input_conv.bias"]))


class Apnet2WeightLoadingRefusalTest(unittest.TestCase):
    ...
    def test_module_of_another_architecture_is_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(TypeError, "APNet2 published weights require Apnet2"):
    ...
    def test_release_with_a_mismatched_digest_is_rejected(self) -> None:
        ...
        module: Apnet2 = Apnet2(Apnet2Config.redmist328())
